"use client";
import { PanelLeft, BookOpen, Menu } from "lucide-react";
...
export function ChatHeader({ onToggleSidebar, collapsedView }: Props) {
    return (
      ...
      dark:bg-white/[0.02] dark:backdrop-blur-2xl dark:border-white/[0.05]">
            ...
            {/* ── Mobile-only hamburger ── visible only below md breakpoint */}
            <button
                type="button"
                onClick={onToggleSidebar}
                className="md:hidden rounded-lg p-1.5 transition-colors text-slate-500
                  hover:bg-black/5 hover:text-slate-800
                  dark:text-neutral-400 dark:hover:bg-white/[0.05] dark:hover:text-neutral-200"
                aria-label="Open navigation menu"
            >
                <Menu size={20} />
            </button>

            {/* ── Desktop collapsed-sidebar re-open button ── hidden on mobile */}
            {collapsedView && (
                <button
                    type="button"
                    onClick={onToggleSidebar}
                    className="hidden md:flex rounded-lg p-1.5 transition-colors text-slate-500
                      hover:bg-black/5 hover:text-slate-800
                      dark:text-neutral-400 dark:hover:bg-white/[0.05] dark:hover:text-neutral-200"
            ...
            {/* ── Logo / title ── always visible so mobile header isn't bare */}
